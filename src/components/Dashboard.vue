...
<div>
  ...
  <div class="col-sm-6">
    <a href="mailto:edflix.project@gmail.com" target="_blank" class="btn btn-warning" style="margin-left:10px"><i class="glyphicon glyphicon-envelope"></i>Feedback</a>
...
<div v-if="userid === null">
  <div class="col-sm-6">
  <Welcome v-if="showModal2" @close="showModal2 = false">
    <h3 slot="header">Welcome EdFlix Dashboard</h3>
  </Welcome>
  </div>
</div>
<div v-if="userid">
 <div class="grid-container" v-if="post.length" style="margin-top:20px;margin-left:20px;">
  <div id="demo" v-for="item in datachak" v-bind:key="item.id">
  <div class="panel panel-default">
    <div class="panel-heading"> 
      <center>
        <div  id="wordlnt">
        <a  data-toggle="tooltip" v-bind:title="item.title" target="_blank" v-bind:href="item.url"><strong> {{item.title}} </strong> </a>
...
<!--       <a v-bind:href="item.url" target="_blank" class="btn btn-info" id="infobtn" data-toggle="tooltip" title="Go to main source of this content"><i class="glyphicon glyphicon-share-alt"></i></a>
 -->      <div class="video">
              <iframe v-bind:src="item.modifyurl" frameborder="0"></iframe>
            </div>
    </div>
    <div class="panel-body">
      <p id="datesize" ><small> {{item.comment}}</small></p>
      <div class="row">
        <div class="col-sm-3" id="labelde" style="background-color: #2FA2F3;border-radius: 3px;">Grade None</div>
        <div class="col-sm-3" id="labelde" style="background-color: #DBAC5C;border-radius: 3px;">No subject</div>
        <div class="col-sm-2" id="labelde" style="background-color: #349E34;border-radius: 3px;">{{item.rating}} <i class="glyphicon glyphicon-star"></i></div>
        <div class="col-sm-3" id="labelde" style="background-color: #CD5B5B;border-radius: 3px;">Uncategorized</div>
...
export default {
  name: 'dashboard',
  components: { FeedBack, newItem, Welcome },
  delay: 1000,
  data () {
    return {
      userid: window.localStorage.getItem('edflix_user_id'),
      showModal: false,
      showModal1: false,
      showModal2: true,
  ...
  computed: {
    datachak: function () {
      if (this.idprams === 'all') {
        console.log(window.localStorage.getItem('edflix_user_id'))
        return this.post
      } else {
