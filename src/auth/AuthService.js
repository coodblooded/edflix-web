          ...
          appService.getIexternalid(user.sub, authResult.accessToken).then(data => {
            if (data === 404) {
              appService.createuseri(user.name, user.email, user.sub, authResult.accessToken).then(data => {
                localStorage.setItem('edflix_user_id', data.user.id)
                var d = new Date()
                d.setTime(d.getTime() + (1 * 1 * 60 * 60 * 1000))
                var expires = 'expires=' + d.toUTCString()
                document.cookie = 'edflix_user_auth' + '=' + data.user.id + ' edflix_user_assess ' + authResult.accessToken + ' edflix_user_assess ' + data.user.name + ';' + expires + ';path=/'
              })
            } else {
              console.log(data)
              ...
              d.setTime(d.getTime() + (1 * 1 * 60 * 60 * 1000))
              var expires = 'expires=' + d.toUTCString()
              document.cookie = 'edflix_user_auth' + '=' + data.id + ' edflix_user_assess ' + authResult.accessToken + ' edflix_user_assess ' + data.name + ';' + expires + ';path=/'
            }
          })
